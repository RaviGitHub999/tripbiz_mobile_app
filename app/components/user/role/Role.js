...
const Role = () => {
  ...
  return (
    <>
                ...
                <View>
                  {
                    approveLoading ?
                      ...
                      tripsData?.filter((a) => {
                        ...
                        var aDate = new Date(a?.requestDetails?.createdAt.seconds * 1000)
                        var bDate = new Date(b?.requestDetails?.createdAt.seconds * 1000)
                        return bDate - aDate
                      })?.map((trip, s) => {
                        var date = (new Date(trip?.tripDetails?.data?.date?.seconds * 1000)).toLocaleString()

                        return (
                          <View style={styles.card} key={trip.approvalRequest.requestId}>
                            <View style={styles.headers}>
                              <Text style={styles.cardTitle}>{trip?.userDetails?.firstName}</Text>
                              <Text style={styles.cardTitle}>{trip?.tripDetails?.data?.name}</Text>
                              <Text style={styles.cardTitle}>Requested on:<Text style={styles.date}>{`  ${date}`}</Text></Text>
                            </View>
                            <View style={styles.tripListCount}>
                              {trip?.tripDetails?.hotels?.length > 0 ? <View style={styles.eachTripList}>
                                <Text style={styles.eachTripListTitle}>{`Hotels - ${trip?.tripDetails?.hotels.length}`}</Text>
                              </View> : null}
                              {trip?.tripDetails?.flights?.length > 0 ? <View style={styles.eachTripList}>
                                <Text style={styles.eachTripListTitle}>{`Flights - ${trip?.tripDetails?.flights?.length}`}</Text>
                              </View> : null}
                            </View>
                            <Text style={styles.priceTitle}>Total price: <Text style={styles.price}> &#8377; {`${trip?.approvalRequest?.totalPrice ? Math.ceil(trip?.approvalRequest?.totalPrice) : ''} `}</Text></Text>
                            <View style={styles.tripListCount}>
                              <Text style={styles.statusTitle}>Status: </Text>
                              <View style={[styles.statusContainer, { backgroundColor: trip?.requestDetails?.status === "Pending" ? "orange" : "green" }]}>
                                <Text style={[styles.eachTripListTitle, { color: colors.white }]}>{trip?.requestDetails?.status}</Text>
                              </View>
                            </View>
                            ...
                            <TouchableOpacity style={styles.DetailsBtn} onPress={() => {
                              ...
                              <Text style={[styles.priceTitle, { color: colors.white }]}>View Details</Text>
                            </TouchableOpacity>
                          </View>
                        )
                      })
      ...
      <PopUp value={openTrip} handlePopUpClose={() => setOpenTrip(false)} customStyles={{ height: Dimensions.get("screen").height / 1.5 }}>
        <ScrollView showsVerticalScrollIndicator={false}>
                    ...
                    <View>
                      <>
                        {trip?.tripDetails?.data?.travellerDetail ? (
                          <>
                            {
                              trip?.tripDetails?.data.travellerDetail[hotel.id]?.map((trav, i) => {
                                var index = i + 1
                                return (
                                  <TravDetails trav={trav} type={"Traveller"} index={index} />
                                )
                              })
                            }
                          </>
                          ...
                          <>
                            {
                              trip?.tripDetails?.data?.travellerDetails[hotel.id]?.map((trav, i) => {
          ...
          <View>
            {trip?.tripDetails?.flights
              ?.map((flight, s) => {
                ...
                var flightArr = [flight?.data[0]?.flight].map(
                  (flight, f) => {
                    return {
                      ...
                      <View>
                        <>
                          {trip?.tripDetails?.data?.travellerDetail ? (
                            ...
                            <>
                              {trip?.tripDetails?.data?.travellerDetails[flight.id]?.map(
                                (trav, i) => {
                                  var type = "Traveller"
...

          {
                        trip?.approvalRequest?.status === "Pending" ? (
                           <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center',gap:10,marginTop:responsiveHeight(1)}}>
                            <Text style={{fontSize:responsiveHeight(2),color:colors.primary,fontFamily:fonts.primary}}>Approve this trip:</Text>
                            <TouchableOpacity style={styles.btn} disabled={loading} onPress={()=>handleApprove(trip?.approvalRequest)}>
                            { loading?<ActivityIndicator color={colors.facebook}/>:<Text style={styles.btnTitle}>Approve</Text>}
                            </TouchableOpacity>
                           </View>
                        ) : (null)
                    }
